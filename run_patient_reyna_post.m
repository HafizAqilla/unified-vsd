% RUN_PATIENT_REYNA_POST
% -----------------------------------------------------------------------
% Post-surgery simulation for the Reyna VSD patient.
%
% PURPOSE:
...
%
% PATIENT:
%   Reyna (3-year-old female | 14.0 kg | 98.0 cm)
%   Same demographics as run_patient_reyna.m, consistent with the calibrated
%   pre-surgery result.
%
% POST-SURGERY HAEMODYNAMIC TARGETS:
%   After successful VSD closure in a child with pre-op Qp/Qs ≈ 1.19 and
%   MAP ≈ 71 mmHg, the expected early post-operative state is:
...
%   adding a 'CalibParams' entry to the clinical.pre_surgery struct.  main_run
%   detects this and uses it in place of the allometric-scaled params0.
%
...
%
% REFERENCES:
%   [1] run_patient_reyna.m         — original pre-surgery patient definition
...
%  USER TOGGLE
% =====================================================================
DO_CALIBRATION = true;  % true  → run full GSA + fmincon calibration (recommended)
                        % false → fast forward simulation only (no GSA, no calibration)

fprintf('=====================================================\n');
fprintf('  POST-SURGERY SIMULATION — run_patient_reyna_post.m\n');
fprintf('=====================================================\n\n');
...
latest_mat = 'C:\Users\asus\Documents\unified-vsd\results\runs\20260522_201100_reyna_pre_surgery\tables\params_calibrated_pre_surgery_early_20260522_201100.mat';

if ~isfile(latest_mat)
    error(['run_patient_reyna_post:noCalibFile\n' ...
           'The specified calibration file was not found:\n  %s\n\n'], latest_mat);
end

fprintf('[run_patient_reyna_post] Loading calibrated parameters from:\n');
fprintf('  %s\n\n', latest_mat);
...
%
%  Patient demographics are IDENTICAL to run_patient_reyna.m.
...
clinical.do_calibration = DO_CALIBRATION;  % gates calibration in main_run Step 6
clinical.do_gsa         = DO_CALIBRATION;  % GSA always mirrors calibration toggle

% ---- Common demographics (same patient, different time point) ----------
clinical.common.patient_name = 'reyna';
clinical.common.age_years  = 3.17;            % [years]
clinical.common.weight_kg  = 14.0;            % [kg]
clinical.common.height_cm  = 98.0;            % [cm]
clinical.common.sex        = 0;               % Female (0)
clinical.common.BSA        = 0.6173419726;    % [m^2]
...
clinical.pre_surgery.VSD_diameter_mm   = 3.025;   % [mm]
clinical.pre_surgery.VSD_gradient_mmHg = 69;      % [mmHg]
...
clinical.pre_surgery.SAP_sys_mmHg      = 100;     % [mmHg]
clinical.pre_surgery.SAP_dia_mmHg      = 57;      % [mmHg]
clinical.pre_surgery.SAP_mean_mmHg     = 71.3;    % [mmHg]
...
fprintf('[run_patient_reyna_post] Warm-start parameters loaded from pre-surgery calibration.\n');
...
%% =====================================================================
fprintf('[run_patient_reyna_post] Launching main_run(''post_surgery'', clinical)...\n\n');
main_run('post_surgery', clinical);

fprintf('\n[run_patient_reyna_post] Post-surgery simulation complete.\n');
