...
rows = {record.arm_id, record.arm_label, record.repeat, record.scaling_mode, ...
    ...
    record.baseline_primary_rmse, record.calibration_status, ...
...
names = {'ArmId','ArmLabel','Repeat','ScalingMode','HistoricalSeedsDisabled', ...
    ...
    'CalibrationStatus','CalibrationAccepted','RunDir'};
if isempty(rows)
...
tbl = table( ...
    ...
    string(rows(:, 16)), logical(cell2mat(rows(:, 17))), string(rows(:, 18)), ...
    'VariableNames', names);
end
