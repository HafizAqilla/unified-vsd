...
%% ====================================================================
% Determine patient population for reference ranges
if isfield(params_pat, 'scaling') && isfield(params_pat.scaling, 'patient') ...
        && isfield(params_pat.scaling.patient, 'age_years') ...
        && params_pat.scaling.patient.age_years < 18
    ref_pop = 'pediatric';
else
    ref_pop = 'adult';
end
ref = clinical_reference_ranges(ref_pop);
clr_ref = [0.35 0.35 0.35];   % dark grey for reference band

fh4 = figure('Name', 'Clinical Metrics', 'NumberTitle', 'off', ...
             'Color', 'w', 'Position', [100 60 1050 700]);
sgtitle(sprintf('Clinical Metrics — Adult vs %s  |  Ref: healthy %s', label_pat, ref_pop), ...
    'FontSize', font_ti+1, 'FontWeight', 'bold');
...
% Reference band: LVEF at x=1, RVEF at x=2
patch(ax4b,[0.5 1.5 1.5 0.5],[ref.LVEF(1) ref.LVEF(1) ref.LVEF(2) ref.LVEF(2)],...
    clr_ref,'FaceAlpha',0.12,'EdgeColor','none');
patch(ax4b,[1.5 2.5 2.5 1.5],[ref.RVEF(1) ref.RVEF(1) ref.RVEF(2) ref.RVEF(2)],...
    clr_ref,'FaceAlpha',0.12,'EdgeColor','none');
...
legend(ax4b,{'Adult ref', label_pat, 'Healthy range'},'Location','southeast','FontSize',7);
...
% Reference bands
patch(ax4e,[0.5 1.5 1.5 0.5],[ref.SVR(1) ref.SVR(1) ref.SVR(2) ref.SVR(2)],...
    clr_ref,'FaceAlpha',0.12,'EdgeColor','none');
patch(ax4e,[1.5 2.5 2.5 1.5],[ref.PVR(1) ref.PVR(1) ref.PVR(2) ref.PVR(2)],...
    clr_ref,'FaceAlpha',0.12,'EdgeColor','none');
...
legend(ax4e,{'Adult ref', label_pat, 'Healthy range'},'Location','northeast','FontSize',7);
grid(ax4e,'on'); box(ax4e,'off');

% --- 4f: Summary text table (with clinical reference column) ---------
ax4f = subplot(2,3,6);
axis(ax4f,'off');
CO_ad = m_ad.LVSV * params_adult.HR / 1000;    % [L/min]  SV[mL] × HR[bpm] / 1000
CO_pt = m_pt.LVSV * params_pat.HR   / 1000;    % [L/min]

% Helper: format a [lo hi] reference range as a compact string
fmt_ref = @(r) helper_fmt_ref(r);

tbl = { ...
    sprintf('%-20s  %6s  %7s  %s', 'Metric', 'Adult', 'Patient', 'Ref (healthy)'); ...
    repmat('-', 1, 56); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'HR [bpm]',        params_adult.HR,       params_pat.HR,       fmt_ref(ref.HR)); ...
    sprintf('%-20s  %6.2f  %7.2f  %s', 'CO [L/min]',      CO_ad,                 CO_pt,               fmt_ref(ref.CO)); ...
    sprintf('%-20s  %6.2f  %7.2f  %s', 'Qp/Qs',           m_ad.QpQs,             m_pt.QpQs,           '1.0 (normal)'); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'LVEDV [mL]',      m_ad.LVEDV,            m_pt.LVEDV,          fmt_ref(ref.LVEDV)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'LVESV [mL]',      m_ad.LVESV,            m_pt.LVESV,          fmt_ref(ref.LVESV)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'RVEDV [mL]',      m_ad.RVEDV,            m_pt.RVEDV,          fmt_ref(ref.RVEDV)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'RVESV [mL]',      m_ad.RVESV,            m_pt.RVESV,          fmt_ref(ref.RVESV)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'LVEF [%%]',        m_ad.LVEF*100,         m_pt.LVEF*100,       fmt_ref(ref.LVEF)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'RVEF [%%]',        m_ad.RVEF*100,         m_pt.RVEF*100,       fmt_ref(ref.RVEF)); ...
    sprintf('%-20s  %6.0f  %7.0f  %s', 'SBP [mmHg]',      m_ad.SAP_max,          m_pt.SAP_max,        fmt_ref(ref.SBP)); ...
    sprintf('%-20s  %6.0f  %7.0f  %s', 'DBP [mmHg]',      m_ad.SAP_min,          m_pt.SAP_min,        fmt_ref(ref.DBP)); ...
    sprintf('%-20s  %6.0f  %7.0f  %s', 'MAP [mmHg]',      m_ad.SAP_mean,         m_pt.SAP_mean,       fmt_ref(ref.MAP)); ...
    sprintf('%-20s  %6.0f  %7.0f  %s', 'LVESP [mmHg]',    m_ad.LVP_max,          m_pt.LVP_max,        fmt_ref(ref.LVESP)); ...
    sprintf('%-20s  %6.0f  %7.0f  %s', 'LVEDP [mmHg]',    m_ad.LVEDP,          m_pt.LVEDP,        fmt_ref(ref.LVEDP)); ...
    sprintf('%-20s  %6.0f  %7.0f  %s', 'RVESP [mmHg]',    m_ad.RVP_max,          m_pt.RVP_max,        fmt_ref(ref.RVESP)); ...
    sprintf('%-20s  %6.0f  %7.0f  %s', 'RVEDP [mmHg]',    m_ad.RVEDP,          m_pt.RVEDP,        fmt_ref(ref.RVEDP)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'PAP_mean [mmHg]', m_ad.PAP_mean,         m_pt.PAP_mean,       fmt_ref(ref.PAP_mean)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'PWP_mean [mmHg]', m_ad.PWP_mean,         m_pt.PWP_mean,       '--'); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'LAP_mean [mmHg]', m_ad.LAP_mean,         m_pt.LAP_mean,       fmt_ref(ref.LAP_mean)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'RAP_mean [mmHg]', m_ad.RAP_mean,         m_pt.RAP_mean,       fmt_ref(ref.RAP_mean)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'SVR [WU]',        m_ad.SVR,              m_pt.SVR,            fmt_ref(ref.SVR)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'PVR [WU]',        m_ad.PVR,              m_pt.PVR,            fmt_ref(ref.PVR)); ...
    sprintf('%-20s  %6.1f  %7.1f  %s', 'Q_VSD [mL/s]',    m_ad.Q_shunt_mean_mLs, m_pt.Q_shunt_mean_mLs, '--') };
text(ax4f, 0.01, 0.97, strjoin(tbl, '\n'), ...
    'FontSize', 6.8, 'FontName', 'Courier New', ...
    'VerticalAlignment', 'top', 'Units', 'normalized');
title(ax4f, sprintf('Summary Table  [Ref: %s]', ref_pop), ...
    'FontSize', font_ti, 'FontWeight', 'bold');
...
function s = helper_fmt_ref(r)
% HELPER_FMT_REF — format a [lo hi] reference range as 'lo – hi'.
% Returns '--' if both values are NaN (metric not specified for population).
if all(isnan(r))
    s = '--';
elseif r(1) == 0
    s = sprintf('<%.4g', r(2));
else
    s = sprintf('%.4g–%.4g', r(1), r(2));
end
end
