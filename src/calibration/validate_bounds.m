...
for row_idx = 1:height(parameter_registry)
    ...
    if row.is_calibratable
        uses_recipe_absolute_bounds = ismember('bound_type', ...
            parameter_registry.Properties.VariableNames) && ...
            strcmp(row.bound_type{1}, 'recipe_absolute_final_bounds');
        if ~uses_recipe_absolute_bounds && ...
                (~isfinite(anchor) || ~(lb < anchor && anchor < ub))
            error('validate_bounds:anchorOutsideBounds', ...
                'Registry anchor must lie inside bounds for calibratable %s.', name);
        end
        if ~uses_recipe_absolute_bounds && ...
                (~isfinite(seeded_value) || ~(lb <= seeded_value && seeded_value <= ub))
